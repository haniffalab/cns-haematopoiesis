    ...
    for n_hidden in "${n_hidden_list[@]}"; do
      for n_hvg in "${n_hvg_list[@]}"; do
        ...
        bsub -q "$q" \
             ...
             -o "${LOG_DIR}/scvi_integration_FIRST_TRIM.${job_name}.%J.out" \
             "module load cellgen/conda; module load cellgen/scvi; \
              python -u /nfs/team298/sm54/BoneAtlasProject/src/integration/10_batch_correction_scvi_haem_best_integration.py  \
                --n_latent $n_latent \
                --n_layers $n_layers \
                --n_hidden $n_hidden \
                --n_hvg $n_hvg"
