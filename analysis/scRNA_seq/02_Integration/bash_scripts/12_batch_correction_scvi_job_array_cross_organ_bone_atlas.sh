  ...
  for n_layers in "${n_layers_list[@]}"; do
    for n_hidden in "${n_hidden_list[@]}"; do
      for n_hvg in "${n_hvg_list[@]}"; do
        ...
        job_name="combined_scvi_${n_latent}_${n_layers}_${n_hidden}_${n_hvg}"
        ...
        bsub -q "$q" \
             ...
             "module load cellgen/conda; module load cellgen/scvi; \
              python -u /nfs/team298/sm54/BoneAtlasProject/src/integration/12_batch_correction_scvi_job_array_cross_organ_bone_atlas.py \
                --n_latent $n_latent \
                --n_layers $n_layers \
                --n_hidden $n_hidden \
                --n_hvg $n_hvg"
