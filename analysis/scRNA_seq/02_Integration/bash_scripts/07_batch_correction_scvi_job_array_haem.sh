...
# ─── Parameter lists ──────────────────────────────────────────────────────
n_latent_list=(30 25 20 15)
n_layers_list=(2  4)
n_hidden_list=(128)
n_hvg_list=(10000 6000 3000)
  ...
  for n_layers in "${n_layers_list[@]}"; do
    for n_hidden in "${n_hidden_list[@]}"; do
      for n_hvg in "${n_hvg_list[@]}"; do
        ...
        job_name="scvi_${n_latent}_${n_layers}_${n_hidden}_${n_hvg}"
        ...
        bsub -q "$q" \
             ...
             -o "${LOG_DIR}/haem_scvi_integration.${job_name}.%J.out" \
             "module load cellgen/conda; module load cellgen/scvi; \
              python -u /nfs/team298/sm54/BoneAtlasProject/src/integration/07_batch_correction_scvi_job_array_haem.py \
                --n_latent $n_latent \
                --n_layers $n_layers \
                --n_hidden $n_hidden \
                --n_hvg $n_hvg"
