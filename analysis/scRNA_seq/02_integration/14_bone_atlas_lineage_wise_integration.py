#!/usr/bin/env python3
"""
Train SCVI per input .h5ad (provided via --h5ad), using HVGs for training only,
then write the SCVI latent embedding back to the FULL object and run neighbors,
UMAP, Leiden, and DE on the FULL AnnData. Save the FULL AnnData and artifacts.

- Accepts a single --h5ad (the .sh loops files)
- Ensures adata.layers['raw_counts'] exists on the FULL object
- HVGs are used only to fit SCVI; embedding is stored in FULL (.obsm["X_scVI"])
- Neighbors/UMAP/Leiden computed on FULL using X_scVI
- DE (wilcoxon) computed on a normalized/log1p COPY; results saved to full.uns
"""
...
parser = argparse.ArgumentParser(
    description="Train SCVI (HVGs) and save embedding + graph/UMAP/Leiden/DE on FULL AnnData."
)
...
parser.add_argument('--data_dir', type=str, default="/nfs/team298/sm54/BoneAtlasProject/data/bone_atlas_anndatas_non_immune",
                    help="Base directory for output subfolders (grouped by cell type).")
...
model_dir = os.path.join(out_base, "trained_scvi_model_full_genes")
umap_dir  = os.path.join(out_base, "umap_plots_full_genes")
data_dir  = os.path.join(out_base, "processed_adata_full_genes")
embed_dir = os.path.join(out_base, "embedding_full_genes")
...
# Ensure raw counts layer exists in FULL object
if 'raw_counts' not in adata_full.layers:
    adata_full.layers['raw_counts'] = adata_full.X.copy()


# --------------------------
# HVG selection on a temporary normalized copy
# (do not alter adata_full.X)
# --------------------------
tmp = adata_full.copy()
...
hvg_kwargs = dict(n_top_genes=args.n_hvg)
...
# --------------------------
# Subset FULL object to HVGs for model training only
# --------------------------
...
# setup_anndata with present keys only
...
# --------------------------
# Neighbors / UMAP / Leiden on FULL object using X_scVI
# --------------------------
...
# --------------------------
# DEG on clusters (normalized/log1p COPY; store in full.uns)
# --------------------------
...
# --------------------------
# Generate UMAP plots (from FULL object)
# --------------------------
for color, fname in [
...
# --------------------------
# Save model and FULL AnnData (all genes)
# --------------------------
